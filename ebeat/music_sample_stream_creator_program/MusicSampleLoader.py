...
class MusicSampleLoader:
    ...
    @staticmethod
    def load_training_samples(sample_path: str, resample_rate: int):
        """
        ...
        """
        sample_files = librosa.util.find_files(sample_path, ext=['wav'])
        sample_files = sorted(sample_files, key=lambda x: int(x.split('_')[-1].split(".")[0]))
        loaded_samples = {}
        for file_path in sample_files:
            loaded_audio_file = AudioSegment.from_wav(file_path)
